...
void showHelp(bool full = false) {
  ...
  std::cout << "  -l, --lang <scm|js|lua>  Specify the lang - Overrides auto "
               "detection"
            << std::endl;
  std::cout << "  -d, --delay <delay>      Specify the delay between ticks"
            << std::endl;
  std::cout << "  -s, --skipIntro          Skip the intro" << std::endl;
  std::cout << "  -t, --hideTurtle         Hide the cursor" << std::endl;
  std::cout << "  -o, --noLoop             Do not loop the program"
            << std::endl;
  std::cout
      << "  -a, --autoClose          Automatically close the program when done"
      << std::endl;
  std::cout << "  -b, --noDebug            Do not show debug messages"
            << std::endl;
  std::cout << "  -g, --noGrid             Do not show the grid" << std::endl;
  std::cout << "  -p, --startPaused        Start the program paused"
            << std::endl;
...
void init(CLIOpts opts) {
  ...
  win =
      SDL_CreateWindow("TurtleLab", 100, 100, 512 * opts.scale,
                       554 * opts.scale, SDL_WINDOW_SHOWN | SDL_WINDOW_OPENGL);
  if (win == nullptr) {
...
void run(CLIOpts opts, RunState state, std::string fileContents = "") {
  ...
  while (true) {
    ...
    if (ts.frame()) {
      ...
      std::string title =
          "TurtleLab - " +
          (state.fileName == "" ? opts.filePath : state.fileName) + " - " +
          std::to_string(ts.delta() - ts.frameDelay) + "ms";
